...
class _BudgetAnalyticsScreenState extends State<BudgetAnalyticsScreen> {
  ...
  Widget _buildSpendingTrendChart() {
    if (_spendingTrends == null || _spendingTrends!.isEmpty) {
      return _buildEmptyCard('No spending data available');
    }

    // Group by month
    final monthlyTotals = <DateTime, double>{};
    for (final trend in _spendingTrends!) {
      monthlyTotals[trend.month] = (monthlyTotals[trend.month] ?? 0) + trend.actual;
    }

    final sortedMonths = monthlyTotals.keys.toList()..sort();
    final maxSpending = monthlyTotals.values.reduce((a, b) => a > b ? a : b);

    // If max spending is 0, show empty state instead of broken chart
    if (maxSpending == 0) {
      return _buildEmptyCard('No spending data available');
    }
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        ...
                        final hasMultipleYears = sortedMonths.first.year != sortedMonths.last.year;

                        // Show year only if data spans multiple years, or always show for clarity
                        final format = hasMultipleYears ? "MMM\nyy" : "MMM\nyy";

                        return Padding(
                          padding: const EdgeInsets.only(top: 8),
                          child: Text(
                            DateFormat(format).format(month),
                            textAlign: TextAlign.center,
                            style: TextStyle(
                              fontSize: 10,
                              height: 1.2,
